...
export class AuthService {
  ...
  async signUp(
    ...
    if (existingUser) {
      throw new BadRequestException(
        'Cette adresse e-mail est déjà utilisé par un autre compte.',
      );
    }
    ...
    await resend.emails.send({
      ...
      text: `Voici le lien pour vérifier votre adresse e-mail : ${process.env.NEXT_PUBLIC_CLIENT_URL}/signup/verify?verification_token=${verificationToken}`,
    });
  ...
  async verifyEmail(token: string): Promise<{ message: string }> {
    ...
    if (user.emailVerifExpires && user.emailVerifExpires < new Date()) {
  ...
  async choosePassword(
    ...
    if (!user.isEmailVerified) {
      throw new UnauthorizedException(
        'Vous devez vérifier votre compte avant de pouvoir choisir un mot de passe.',
      );
    }
  ...
  async signIn(
    ...
    if (!user.isEmailVerified) {
      throw new UnauthorizedException(
        "Merci de bien vouloir vérifier votre compte pour pouvoir accéder à l'application.",
      );
    }
